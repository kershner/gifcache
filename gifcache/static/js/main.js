...
function colorPageElements() {
    ...
    colorWave('.colorwave-speed', $('#feature-speed').css('color'));
    colorWave('.colorwave-dailygifs', $('#feature-dailygifs').css('color'));
    colorWave('.colorwave-validation', $('#feature-validation').css('color'));
...
// Subtle fade-in of elements on profile view page
function profileFadeIn() {
    setTimeout(function() {
        $('.profile-info, .viewer-tag-title').css('opacity', '1.0');
    }, 100);
    setTimeout(function() {
        $('.inner-nav, .viewer-input, .viewer-blurb').css('opacity', '1.0');
    }, 150);
    setTimeout(function() {
...
function profileColorWave() {
    colorWave('.profile-name', '#FFFFFF');
    colorWave('.title', '#FFFFFF');
    colorWave('.colorwave-gifgrabber', '#FFFFFF');
...
function colorProfile() {
    ...
    $('.profile-info').css({'background-color': profileInfoColor});
    $('.profile-avatar img, .profile-avatar video').css({'background-color': profileInfoColor});
...
function gifGrabberSuggestions() {
    ...
    var suggestions = ['gifs', 'gifrequests', 'makemeagif', 'physicsgifs', 'perfectloops',
                        ...
                        'wastedgifs'];
    var choices = shuffle(suggestions);
    var html =  '<div class="suggestion animate-fast">' + choices[0] + '</div>' +
                '<div class="suggestion animate-fast">' + choices[1] + '</div>' +
                '<div class="suggestion animate-fast">' + choices[2] + '</div>';
...
function grabberLightbox(url, title) {
    ...
    var element = '';
    if (extension === 'gif') {
        element = '<img class="grabber-expanded" src="' + url + '">';
    } else if (extension === 'mp4') {
        element = '<video src="' + url + '" autoplay loop class="grabber-expanded" poster="../static/img/preload.gif"></video>';
    }
    var titleElement = '<div class="label grabber-lightbox-label">' + title + '</div>';
    var html = element + titleElement;
    $('.grabber-lightbox').append(html).removeClass('hidden');
...
function updateSelectedGifs() {
    ...
    if (selectedCount > 0) {
        ...
        $('.selected-gifs-number').text(selectedCount).css('background-color', color);
    } else {
...
function gifGrabberTeardown() {
    $('.holder').empty();
    $('#grabber-results, #grabber-error').empty();
    $('.gifgrabber-results, #grabber-error').addClass('hidden');
...
function gifGrabberResults(json) {
    var shuffledColors = shuffle(COLORS);
    var counter = 0;
    var allowed = ['.gifv', '.mp4', '.webm'];
    var grabberDataHtml =   '<div class="grabber-data-container">' +
                            '<div class="grabber-data gif-number animate" style="background-color: ' +
                            shuffledColors[0] + '">' + json.number_gifs + '<div class="grabber-data-label">' +
                            'GIFs Found</div></div><div class="grabber-data subreddit-name animate" style="background-color: ' +
                            shuffledColors[1] + '">' + json.subreddit + '<div class="grabber-data-label">Subreddit</div></div>' +
...
// Hits GfyCat API for proper MP4 url and updates the element's src attribute
function getGfyMp4(gifId, gfyName) {
    var baseUrl = 'http://gfycat.com/cajax/get/';
    $.ajax({
        url: baseUrl + gfyName,
        type: 'POST',
        success: function(json) {
            var mp4url = json.gfyItem.mp4Url;
            var elementId = '#grabber-gif-' + gifId;
            $(elementId).find('video').attr('src', mp4url);
        },
        error: function(xhr, errmsg, err) {
            console.log('Error!');
        }
...
function hoverGifs() {
    ...
    $('.gif-grid-element').on({
        mouseenter: function() {
            ...
            if (extension === 'gif') {
                html = '<div class="img-wrapper animate"><img src="' + gifUrl + '"></div>';
            }
            else if (isGifv || isMp4 || isWebm || isGfycat) {
                html = '<div class="img-wrapper animate"><video src="' + gifUrl + '" autoplay loop poster="../static/img/preload.gif"></video></div>';
            }
...
function addTags() {
    $('.add-tags-title').on('click', function(e) {
        ...
        if (clicked) {
            ...
            var html =  '<input class="add-tag-field" maxlength="20" type="text" placeholder="tag">' +
                        '<div class="add-tag-submit btn blue-btn">Add</div>';
...
function showInnerNav() {
    $(document).on('scroll', function() {
        ...
        if (scroll > 390) {
            ...
            $('.inner-nav-profile-info').removeClass('hidden').addClass('shown');
            $('#inner-nav-break').removeClass('hidden');
        } else {
            $('.inner-nav').removeClass('inner-nav-fixed');
            $('.inner-nav-profile-info').removeClass('shown').addClass('hidden');
            $('#inner-nav-break').addClass('hidden');
        }
    });
}
...
function gridView() {
    $('.display-grid-icon').on('click', function() {
        ...
        var randomnumber = (Math.random() * (COLORS.length + 1)) << 0;
        var randomnumber1 = (Math.random() * (COLORS.length + 1)) << 0;
...
function populateGrid(icon) {
    ...
    $(tagGroup).find('.gif-grid-element').each(function() {
        ...
        if (extension === 'gif') {
            element = '<div class="grid-img-wrapper animate-fast"><img src="' +
                            url + '" class="animate"><div class="label grabber-lightbox-label' +
                            labelClass + '">' + title + '</div></div>';
        } else if (extension === 'mp4') {
            element = '<div class="grid-img-wrapper animate-fast"><video src="' +
...
function partyModeToggle() {
    $('.party-mode-icon').on('click', function() {
        ...
        var html =  '<div class="lightbox party-mode-wrapper animate-slow hidden">' +
                    ...
                    '<i class="fa fa-times"></i></div><div class="lightbox-tag-title lobster">' + tagTitle +
                    '<div class="lightbox-gif-count">' + totalGifs + ' GIFs</div></div>' +
...
function partyMode(tagGroup) {
    ...
    $(tagGroup).children('.gif-grid-element').each(function() {
        ...
        if (extension === 'gif') {
            element = '<div class="party-img-wrapper"><img src="' + url +
                        '" class="animate"><div class="label grabber-lightbox-label' +
                        labelClass + '">' + title + '</div></div>';
        } else if (extension === 'mp4') {
            element = '<div class="party-img-wrapper"><video src="' + url +
...
function validationResults(data) {
    ...
    if (dupes.length > 0) {
        ...
        for (i=0; i<dupes.length; i++) {
            ...
            if (extension === 'gif') {
                image = '<div class="img-wrapper"><img src="' + url + '"></div>';
            ...
            dupesElement =  '<div class="validation-result animate-fast"><div class="validation-result-inner-wrapper hidden"></div>' +
                            '<div class="validation-id hidden">' + dupes[i][0] +
                            '</div>' + image + '<div class="validation-label">' + dupes[i][2] +
                            '</div>' + tagsElement + '</div>';
            dupesHtml += dupesElement;
        }
        dupeFinalHtml = '<div class="tag-manager-section"><div class="validation-results-section-title">Duplicates</div>' + dupesHtml + '</div>';
        $('.validation-results').append(dupeFinalHtml);
    } else {
        dupeFinalHtml = '<div class="tag-manager-section"><div class="validation-results-section-title">' +
                    'Duplicates</div><div class="no-results">None of your GIFs are duplicates</div>';
        $('.validation-results').append(dupeFinalHtml);
    }
    // 404 Logic
    var notFounds = data['404s'];
    var notFoundsFinalHtml = '';
    if (notFounds.length > 0) {
        ...
        for (i=0; i<notFounds.length; i++) {
            ...
            var notFoundsTags = [];
            var notFoundsTagElement = '';
            var notFoundsElement = '';
            ...
            notFoundsElement =  '<div class="validation-result animate-fast"><div class="validation-result-inner-wrapper hidden"></div>' +
                                ...
                                '</div>' + notFoundsTagElement + '</div>';
            notFoundsHtml += notFoundsElement;
        }
        notFoundsFinalHtml = '<div class="tag-manager-section"><div class="validation-results-section-title">404s</div>' + notFoundsHtml + '</div>';
        $('.validation-results').append(notFoundsFinalHtml);
    } else {
        notFoundsFinalHtml = '<div class="tag-manager-section"><div class="validation-results-section-title">' +
...
// Binds css scaling to range element on viewer page
function viewerResize() {
    $('#viewer-slider').on('input', function() {
        var scaleValue = $(this).val();
